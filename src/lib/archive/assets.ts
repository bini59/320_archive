...
const EXTENSIONS = {"image/jpeg":"jpg","image/png":"png","image/gif":"gif","image/webp":"webp","image/avif":"avif","application/pdf":"pdf","text/plain":"txt","text/css":"css","font/woff":"woff","font/woff2":"woff2","font/ttf":"ttf","font/otf":"otf","application/font-woff":"woff","application/vnd.ms-fontobject":"eot"} as const;
...
export function assetPathFor(id:string,asset:CapturedAsset):string{return `/archives/${encodeURIComponent(id)}/assets/${capturedAssetKey(asset)}`;}
...
export function rewriteAssetReferences(html:string,baseUrl:string,archiveId:string,assets:CapturedAsset[]):string {
  ...
  const byUrl=new Map<string,CapturedAsset>();
  for (const asset of assets) {
    byUrl.set(asset.originalUrl, asset);
    byUrl.set(asset.finalUrl, asset);
  }
  ...
  return html.replace(/<(img|a)\b[^>]*>/gi,(tag:string,name:string)=>{
    if(name.toLowerCase()==="img"){
      ...
      let next=replaceAttribute(tag,"src",captured?assetPathFor(archiveId,captured):null);
      const srcset=attribute(tag,"srcset");
      if(srcset){const entries=srcset.split(",").map(part=>{const bits=part.trim().split(/\s+/);const url=resolve(bits[0]);const asset=url?byUrl.get(url):undefined;return asset?`${assetPathFor(archiveId,asset)}${bits.slice(1).length?` ${bits.slice(1).join(" ")}`:""}`:null;}).filter(Boolean);next=replaceAttribute(next,"srcset",entries.length?entries.join(", "):null);}
      return next;
    }
    ...
    if(!candidate)return tag;const captured=byUrl.get(resolved);return replaceAttribute(tag,"href",captured?assetPathFor(archiveId,captured):null);
  });
}
