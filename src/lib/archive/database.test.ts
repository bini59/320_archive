...
const roots:string[]=[]; const repos:SqliteArchiveRepository[]=[];
async function fixture(){const root=await mkdtemp(path.join(os.tmpdir(),"archive-db-"));roots.push(root);const repo=new SqliteArchiveRepository(path.join(root,"db.sqlite"));repos.push(repo);return {root,repo};}
afterEach(async()=>{repos.splice(0).forEach(r=>r.close());await Promise.all(roots.splice(0).map(r=>rm(r,{recursive:true,force:true})))});
...
describe("SqliteArchiveRepository public library",()=>{
  ...
  it("migrates idempotently and supports two connections",async()=>{const {root,repo}=await fixture();const second=new SqliteArchiveRepository(path.join(root,"db.sqlite"));repos.push(second);const row=repo.createOrGet({originalUrl:"https://example.com/a",normalizedUrl:"https://example.com/a"}).archive;second.markSaved(row.id,snapshot("Shared"),"multi connection");expect(repo.listPublic({q:"multi"}).total).toBe(1);});
  it("rolls back budget, saved state, tags and FTS together when finalization fails",async()=>{const {repo}=await fixture();const limits={windowMs:60_000,maxSubmissions:10,maxStoredBytes:10,reserveBytes:10,timeoutMs:1_000};const reservation=repo.reserveBudget(limits)!;expect(()=>reservation.finalizeSaved({archiveId:"missing",snapshot:snapshot("Must rollback"),indexText:"must-not-index",tags:[{name:"News",slug:"news"}],byteLength:10})).toThrow("Archive is not pending");reservation.release();expect(repo.listPublic({q:"must-not-index",tag:"news"}).total).toBe(0);expect(repo.reserveBudget(limits)).not.toBeNull();});
  it("releases a reservation when actual bytes exceed the final quota", async () => {
    const { repo } = await fixture();
    const limits = { windowMs: 60_000, maxSubmissions: 10, maxStoredBytes: 10, reserveBytes: 10, timeoutMs: 1_000 };
    const row = repo.createOrGet({ originalUrl: "https://example.com/too-large", normalizedUrl: "https://example.com/too-large" }).archive;
    const reservation = repo.reserveBudget(limits)!;

    expect(reservation.finalizeSaved({
      archiveId: row.id,
      snapshot: snapshot("Too large"),
      indexText: "not indexed",
      tags: [],
      byteLength: 11,
    })).toBeNull();
    expect(repo.reserveBudget(limits)).not.toBeNull();
  });
  it("treats malformed tag filters as an empty safe filter boundary",async()=>{const {repo}=await fixture();const row=repo.createOrGet({originalUrl:"https://example.com/a",normalizedUrl:"https://example.com/a",tags:[{name:"News",slug:"news"}]}).archive;repo.markSaved(row.id,snapshot("Saved"));expect(repo.listPublic({tag:"NEWS"}).total).toBe(1);expect(repo.listPublic({tag:"news' OR 1=1 --"}).total).toBe(1);});
});
