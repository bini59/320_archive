...
`pnpm test`는 Node 환경에서 단위·통합 테스트를 실행합니다. `pnpm test:e2e`를 처음 실행하기 전에는 `pnpm exec playwright install --with-deps chromium`으로 브라우저와 OS dependencies를 설치해야 합니다. E2E 서버는 실행마다 운영 데이터와 분리된 임시 SQLite 및 아카이브 경로를 사용하며, 외부 인터넷 대신 테스트 전용 HTML fixture를 캡처합니다.
...
| `ARCHIVE_STORAGE_ROOT` | `data/archives` | UUID별 `original.html`, `readable.html`, 선택적 `rendered.html`, `assets.json`을 저장할 디렉터리 |
...
| `ARCHIVE_ASSET_MAX_BYTES` | `10485760` | 단일 image/attachment/CSS/font 자산의 최대 바이트 수 |
...
제출 요청은 최대 10초 동안 같은 Server Action 안에서 동기 처리됩니다. HTTP(S) URL만 허용하며 credentials와 8 KiB 초과 URL을 거절합니다. 각 요청과 리디렉션 hop마다 DNS A/AAAA 결과 전체가 global-unicast인지 검사한 뒤, 기존 fetch 경로는 검증한 IP로 실제 연결을 고정합니다. 브라우저 캡처 경로도 navigation, redirect, stylesheet, image, font, fetch/XHR를 매번 같은 public URL 정책으로 재검증하고, WebSocket·beacon·ping·form submit·popup·child frame·media/streaming은 차단합니다. 응답은 `text/html` 또는 XHTML이어야 하며 Content-Length와 실제 압축 해제 스트림 모두에 5 MiB 제한을 적용합니다.

CSR 페이지는 보존 시점의 격리된 Playwright Chromium context에서 `domcontentloaded`, bounded `networkidle`, `document.fonts` readiness 후의 DOM을 캡처합니다. 캡처 context에는 쿠키·storage state·auth header를 주입하지 않습니다. 저장된 `rendered.html`은 실행 가능한 script를 포함하지 않고, inline style은 self-hosted CSS asset으로 이동하며, CSS `@import`/`url(...)`, font, image는 archive manifest의 digest 경로로만 연결됩니다. 원격 fallback을 찾지 못하거나 제한을 초과한 자산은 결과에서 제거합니다.

성공한 아카이브는 `<archive-root>/<uuid>/original.html`에 받은 원문 바이트를, 선택적으로 `rendered.html`에 post-hydration 표현을, `snapshot.json`에 제목·설명·최종 URL·캡처 시각·원문 바이트 수를 저장합니다. CSS/font/image/attachment는 `assets.json`과 digest-keyed `assets/` 아래에 함께 저장합니다. 모든 파일을 staging directory에 fsync한 뒤 atomic rename하고, 그 뒤에만 SQLite 상태를 `saved`로 바꿉니다. 실패한 아카이브에는 네트워크 주소나 내부 예외 대신 허용 목록의 안전한 사유만 공개합니다.
...
이미지는 다음 경로를 사용합니다. compose 환경 변수나 별도 실행 환경에서도 두 경로를 동일한 영속 volume 안에 두어야 합니다. Debian slim 기반 이미지에 ARM64 Playwright Chromium과 필요한 OS libraries를 함께 설치하므로 Alpine 기반 실행 이미지를 사용하지 않습니다.
...
이미지 내부에서 production browser가 정상인지 확인하려면 컨테이너가 실행된 뒤 다음 smoke를 사용할 수 있습니다.

```bash
docker exec "$(docker compose ps -q app)" node --input-type=module -e \
...
변경된 코드가 포함된 PR은 GitHub-hosted ARM64 runner에서 lint, typecheck, 단위·통합 테스트,
Playwright E2E, production build, Docker build와 final-image Chromium launch smoke를 실행합니다. Markdown 및 `docs/`만 변경된 PR은 검증 workflow를 건너뜁니다. 변경된 코드가 포함된 `main` commit은
